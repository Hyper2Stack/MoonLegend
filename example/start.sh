...
if [ $? -ne 0 ]; then
  sudo docker run -d --name moonlegend_db -p 3306:3306 -e MYSQL_ROOT_PASSWORD=root daocloud.io/mysql:5.5 > /dev/null
  sleep 15
...
sed -e "s/127\.0\.0\.1/${IP}/g" ${ROOT_DIR}/config/moonlegend.json > /tmp/moonlegend.json
...
echo "moonlegend mysql:  root:root@tcp(${IP}:3306)/moonlegend"
